...
os.makedirs(DATA_DIR, exist_ok=True)  # <-- ensure folder exists at import time
DB_PATH = os.path.join(DATA_DIR, "wellatlas.db")

app = Flask(__name__)

def db():
    # Double-check dir exists before each connect (defensive)
    if not os.path.isdir(DATA_DIR):
        os.makedirs(DATA_DIR, exist_ok=True)
    return sqlite3.connect(DB_PATH, check_same_thread=False, detect_types=sqlite3.PARSE_DECLTYPES)

def ensure_schema():
    c = db(); c.row_factory = sqlite3.Row; cur = c.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS customers(id INTEGER PRIMARY KEY, name TEXT UNIQUE, phone TEXT, email TEXT)")
...
def api_sites():
    c=db(); c.row_factory = sqlite3.Row; cur=c.cursor()
    cur.execute("SELECT sites.*, customers.name as customer FROM sites JOIN customers ON customers.id=sites.customer_id")
    rows=[{k:row[k] for k in row.keys()} for row in cur.fetchall()]
    c.close(); return jsonify(rows)
